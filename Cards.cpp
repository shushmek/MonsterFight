	...
	:_type(type), _name(name), _costAP(costAP), _costCP(costCP), _minBet(min), _maxBet(max), Button(position, texturePath, buttonText, fontPath, clickSoundPath, textCol, size) 
{
	...
	else
	{
		...
		_costCPObj->_sprite->setColor(Color(255, 200, 0));
...
void Cards::setEffectPosition(Vector2f pos)
{
	_effect->setPosition(pos);
}

void Cards::setEffectTextPosition(Vector2f pos)
{
	_effectT->setPosition(pos);
}
